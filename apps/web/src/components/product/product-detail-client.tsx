...
import { priceDokumentasi, priceJeep, priceTransport, priceVilla } from "@/lib/pricing";
import { ProductDetailNav } from "./product-detail-nav";
...
export function ProductDetailClient({
  ...
  // (opsional) gallery real dari BE nanti
  ...
  const totalAmount = useMemo(() => {
    ...
    if (product.type === "jeep") return priceJeep({ pricePerHour: product.price, hours }).subtotal;
    if (product.type === "dokumentasi") return priceDokumentasi({ pricePerHour: product.price, hours }).subtotal;
    if (product.type === "transport") return priceTransport({ pricePerRoute: product.price }).subtotal;
    return undefined;
  ...
  const canAdd = useMemo(() => {
    if (product.type === "villa")
      return Boolean(stay.start && stay.end && daysBetween(stay.start, stay.end) > 0);
  ...
  // scroll ke blok booking dari tombol “Select …”
  ...
  return (
    <>
      {/* Sticky detail nav + search; muncul saat scroll melewati hero */}
      <ProductDetailNav product={product} initialQuery={initialQuery} />
      ...
      <div className="container-page py-6">
        ...
        <section id="location" className="card mt-6 scroll-mt-28">
          ...
          <div className="mt-3 h-48 rounded-xl border border-[var(--line)] skel" aria-hidden />
        ...
        <section id="facilities" className="card mt-6 scroll-mt-28">
          ...
          {product.amenities?.length ? (
            <ul className="grid grid-cols-2 md:grid-cols-3 gap-2 text-sm text-[var(--muted)]">
              {product.amenities.map((a, i) => <li key={i}>• {a}</li>)}
            </ul>
          ) : (
            <p className="text-sm text-[var(--muted)]">Fasilitas detail akan ditambahkan.</p>
          )}
        </section>
        ...
        <section id="policy" className="card mt-6 scroll-mt-28">
          ...
          <div className="text-sm text-[var(--muted)] space-y-1">
            ...
            <div>General instructions: bawa KTP/ID yang berlaku saat check-in.</div>
          </div>
        </section>
        ...
        <section id="reviews" className="card mt-6 scroll-mt-28">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              { name: "Nadia", text: "Bersih, staff ramah, sunrise jeep mantap!", stars: 5 },
