...
export function ProductDetailNav({ product, initialQuery }: Props) {
  ...
  // tampil saat scroll melewati hero/galeri
  ...
  // kapasitas dasar sebagai info untuk villa
  const baseCapacity = useMemo(
    () => (product.type === "villa" ? product.baseCapacity ?? 2 : 0),
    [product]
  );
  ...
  function onSearch() {
    ...
    if (product.type === "villa") {
      ...
      q.set("adults", String(baseCapacity));
    } else {
      q.set("date", date);
      q.set("time", time);
    }
            ...
            {product.type === "villa" ? (
              <>
                ...
                <label
                  ...
                  title="Guest mengikuti kapasitas dasar (ubah via add-on)"
                >
                  <span aria-hidden={true}>👤</span>
                  <input
                    className="bg-transparent text-sm w-full"
                    value={`${baseCapacity} tamu (dasar)`}
                    disabled
                    aria-disabled={true}
                  />
                </label>
